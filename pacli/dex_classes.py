...
class Dex:
    ...
    @classmethod
    def create_offer(self, deckid: str, amount: int, lock: int, lockaddr: str, sign: bool=False, send: bool=False):
        # create_offer locks the card on the same address than was sent, thus receiver is Settings.key.address
        return dxu.card_lock(deckid=deckid, amount=amount, lock=lock, lockaddr=lockaddr, sign=sign, send=send)
    ...
    def show_locks(self, deckid):
        ...
        state = dxu.pa.protocol.DeckState(cards)
        return state.locks
